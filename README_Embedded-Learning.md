...
# Catalog
第一章 [编程语言](#programming-languages)
第二章 [编程基础](#programming-fundamentals)
第三章 [操作系统](#operating-systems)
第四章 [微控制器](#microcontrollers)
第五章 [接口与协议](#interfaces--protocols)
...
编译：预处理器（preprocessor）&rarr; 解析（parse）&rarr; 优化器（optimizer）
预处理器 &rarr; 执行文本操作，替换
解析 &rarr; 产生绝大多数错误和警告
...
```c
/* GCC基本编译命令 */
gcc hello.c -o hello         // 编译C文件
gcc -c hello.c               // 仅编译，不链接
gcc -S hello.c               // 生成汇编代码
gcc -E hello.c -o hello.i    // 仅执行预处理
/* 代码优化 */
gcc -O2 hello.c -o hello     // 优化编译
/* 调试相关 */
gcc -g hello.c -o hello      // 生成调试信息
/* 其他常用选项 */
gcc -Wall hello.c -o hello   // 显示所有警告
gcc hello.c -o hello -lm     // 使用特定库：-lm 选项用于链接数学库。
```
...
static int c( int d ){}
```
**全局变量**和未使用`static`关键字修饰的**函数**默认具有外部链接属性，可被其他文件通过`extern`关键字引用。
...
- 逗号操作符
  `expression1, expression2, ... , expressionN`：对每个表达式自左向右逐个求值，整个逗号表达式的值就是最后那个表达式的值。
...
`NULL`指针，不指向任何东西，可以赋 0 值。对`NULL`指针解引用**非法**。
